...
function patchInclusionParent() {
  ...
  const definitionHeading='<h2 id="pengertian">Apa Itu Inklusi Sosial?</h2>';
  const nextOriginal='<p><strong>Inklusi sosial adalah</strong>';
  const definitionStart=html.indexOf(definitionHeading);
  const originalStart=html.indexOf(nextOriginal,definitionStart);
  if(definitionStart>=0&&originalStart>definitionStart){
    const answer='<p data-catchup="inclusion-definition"><strong>Jawaban singkat:</strong> Inklusi sosial adalah proses menghilangkan hambatan agar setiap orang dapat mengakses sumber daya, berpartisipasi, membangun relasi, diakui, dan memengaruhi keputusan. Kehadiran saja belum cukup. Orang perlu memperoleh informasi yang dapat diakses, pilihan yang nyata, rasa aman, peran bermakna, dan mekanisme untuk mengubah layanan.</p>';
    html=html.slice(0,definitionStart)+definitionHeading+answer+html.slice(originalStart);
  }
...
function patchOrganizationPages() {
  ...
  for(const rel of ['index.html','tentang.html']){
    ...
    if(rel==='index.html'){
      const entitySpan='<span class="title-line">Yayasan Ukhuwah Kaffah Amanatullah</span>';
      while(html.includes(entitySpan+entitySpan))html=html.replace(entitySpan+entitySpan,entitySpan);
      if(!html.includes(entitySpan))html=html.replace('<span class="title-line">Membangun</span>',entitySpan+'<span class="title-line">Membangun</span>');
    }else{
