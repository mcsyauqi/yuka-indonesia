...
function updateBlogIndex() {
  ...
  const gridMarker = '<div class="blog-grid" id="blogGrid">';
  const cards = articles.filter((article) => !blog.includes(`artikel/${article.slug}`)).map((article) => `
                <!-- Article: ${article.title} -->
                <article class="card blog-card animate-on-scroll" data-yuka-revision="yuka-autopilot-2026-09-05-${article.slug}"><div class="card-image"><img src="${article.image}" alt="${article.alt}" loading="lazy"></div><div class="card-body"><span class="card-category">Pendidikan</span><h3 class="card-title"><a href="artikel/${article.slug}">${article.title}</a></h3><p class="card-text">${article.description}</p><div class="card-meta"><span>05 Sep 2026</span><span>12 menit baca</span></div></div></article>`).join('');
  if (cards && blog.includes(gridMarker)) blog = blog.replace(gridMarker, `${gridMarker}${cards}`);

  fs.writeFileSync(blogPath, blog);
  console.log(`blog-index-updated=${Boolean(cards)}`);
}
